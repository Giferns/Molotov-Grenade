...
new const SRVCMD_BONUSMENU_RBS[] = "molotov_bmrbs"

// Имя файла основного конфига в 'amxmodx/configs'
new const CFG_FILENAME[] = "plugins/molotov_bonusmenu_rbs.cfg"
...
public plugin_init() {
	register_plugin("[Molotov] BonusMenu RBS", PLUGIN_VERSION, "mx?!")
...
RegCvars() {
	bind_pcvar_num( create_cvar("molotov_bmrbs_only_buyzone", "1",
		.description = "Разрешить покупать гранату только в зоне закупки (1/0) ?" ), g_iBuyzone );

	bind_pcvar_num( create_cvar("molotov_bmrbs_dm_mode", "1",
...
public srvcmd_GiveItem_BonusMenuRBS() {
	...
	if(bitFlag && szFlag[0] != '0' && !(get_user_flags(pPlayer) & bitFlag)) {
		client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_NO_ACCESS")
		return PLUGIN_HANDLED
	}

	if(!is_user_alive(pPlayer)) {
		client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_ONLY_ALIVE")
		return PLUGIN_HANDLED
	}

	if(IsUserHasMolotov(pPlayer)) {
		client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_ALREADY_HAVE")
		return PLUGIN_HANDLED
	}
	...
	if(g_iBuyzone && !rg_get_user_buyzone(pPlayer)) {
		client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_ONLY_BUYZONE")
		return PLUGIN_HANDLED
	}
	...
	if(iPrice) {
		if(bonusmenu_get_user_points(pPlayer) < iPrice) {
			client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_NOT_ENOUGH_POINTS")
			return PLUGIN_HANDLED
		}
	...
	GiveUserMolotov(pPlayer, iCount, iCount)
...
bool:CheckCooldown(pPlayer) {
	if(!g_iDmMode) {
		if(g_iCooldown[pPlayer]) {
			new szRounds[32]
			func_GetEnding(g_iCooldown[pPlayer], "MOLOTOV_BMRBS_ROUNDS_1", "MOLOTOV_BMRBS_ROUNDS_2", "MOLOTOV_BMRBS_ROUNDS_3", szRounds, charsmax(szRounds))
			client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_WAIT_MORE_ROUNDS", g_iCooldown[pPlayer], szRounds)
			return false
		}
	...
	client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_WAIT_MORE_SECONDS", iSeconds / 60, iSeconds % 60)
	return false
}
...
bool:CheckMinRound(pPlayer, iMinRound) {
	if(!g_iDmMode) {
		if(rg_get_current_round() < iMinRound) {
			client_print_color(pPlayer, print_team_red, "%l", "MOLOTOV_BMRBS_MIN_ROUND", iMinRound)
			return false
		}
